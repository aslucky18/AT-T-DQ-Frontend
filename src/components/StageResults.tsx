...
// Type definitions
interface KPIData {
  num_integer_columns?: number;
  num_float_columns?: number;
  num_string_columns?: number;
  num_boolean_columns?: number;
  num_date_columns?: number;
  num_other_columns?: number;
  completeness?: number;
  consistency?: number;
  accuracy?: number;
...
export const StageResults = ({
  ...
  if (!data || !data.kpis || !data.summary_table) return null;
  ...
  const getChartData = () => {
    if (stageNumber === 1) {
      ...
      return [
        { name: "Completeness", value: data.kpis.completeness, color: "#3b82f6" },
        { name: "Consistency", value: data.kpis.consistency, color: "#10b981" },
        { name: "Accuracy", value: data.kpis.accuracy, color: "#f59e0b" }
      ];
    } else {
      return [
                ...
                <PieChart>
                  ...
                  >
                    {getChartData().map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.value?.toString() || COLORS[index % COLORS.length]}
                      />
                    ))}
                  </Pie>
                  <Tooltip />
      ...
      <Card>
        ...
        <CardContent>
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <code className="text-sm bg-white p-2 rounded border block whitespace-pre overflow-x-auto">
              {data.summary_table}
            </code>
          </div>
        </CardContent>
